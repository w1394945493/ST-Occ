...
class VoxelFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar=4, num_points_in_voxel=1,
        ...
        if uncertainty_head is not None:
            uncertainty_head = dict(uncertainty_head)
            uncertainty_head.pop('type', None)
            from mmdet3d.models.stocc.modules.uncertainty import UncertaintyEstimator_w_feature_au
            self.uncertainty_head = UncertaintyEstimator_w_feature_au(**uncertainty_head)
        else:
    ...
    def forward(self,
                ...
                *args,
                ...
                shift=0.,
                ref_points=None,
                transformed_ref_points=None,
                uncertainty_embed=None,
                prev_weights=None,
                gt_flow=None,
                au=None,
                **kwargs):
        ...
        if gt_flow is not None:
            # if len(gt_flow.shape) == 4:
            #     H, W, D, C = gt_flow.shape
            #     for i in range(H):
            #         for j in range(W):
            #             new_flow[i, j, :, 0] = - gt_flow[j, H-i-1, :, 1]
            #             new_flow[i, j, :, 1] = gt_flow[j, H-i-1, :, 0]
            #     gt_flow = new_flow.unsqueeze(0).permute(0, 4, 3, 1, 2)
            # elif len(gt_flow.shape) == 5:
            #     B, H, W, D, C = gt_flow.shape
            #     for i in range(H):
            #         for j in range(W):
            #             new_flow[:, i, j, :, 0] = - flow[:, j, H-i-1, :, 1]
            #             new_flow[:, i, j, :, 1] = flow[:, j, H-i-1, :, 0]
            #     gt_flow = new_flow.permute(0, 4, 3, 1, 2)
            # else:
            #     raise ValueError(
            #         'dim of gt_flow ({}) != 4 or 5'.format(len(gt_flow.shape)))
            if len(gt_flow.shape) == 4:
                H, W, D, C = gt_flow.shape
                gt_flow = gt_flow.unsqueeze(0).permute(0, 4, 3, 1, 2) # BS C D H W
            elif len(gt_flow.shape) == 5:
                B, H, W, D, C = gt_flow.shape
                gt_flow = gt_flow.permute(0, 4, 3, 1, 2)
            else:
                raise ValueError(
                    'dim of gt_flow ({}) != 4 or 5'.format(len(gt_flow.shape)))
            new_H, new_W, new_D = H // 2, W // 2, D // 2
            gt_flow_resized = F.interpolate(gt_flow, size=(new_D, new_H, new_W), mode='trilinear', align_corners=False) # BS C D/2 H/2 W/2
            flow_down_sampling = gt_flow_resized.permute(0, 3, 4, 2, 1) # BS H/2 W/2 D/2 C
            
            # if self.correct_flow:
            #     flow_down_sampling = flow_down_sampling.permute(0,3,2,1,4) # BS H/2 W/2 D/2 C
            
            flow_down_sampling = flow_down_sampling.reshape(1, -1, C) # BS num_querys C=2 # num_query = D * W * H
        else:
            flow_down_sampling = None
        ...
        if ref_points is not None:
            ...
            ref_2d = self.get_reference_points(
                bev_z, bev_h, bev_w, self.num_points_in_voxel, dim='2d', bs=bev_query.size(1),  device=bev_query.device, dtype=bev_query.dtype)

        if transformed_ref_points is not None:
            shift_ref_2d = transformed_ref_points
        else:
            shift_ref_2d = self.get_reference_points(
                bev_z, bev_h, bev_w, self.num_points_in_voxel, dim='2d', bs=bev_query.size(1),  device=bev_query.device, dtype=bev_query.dtype)
            # raise ValueError('transformed_ref_points is None')
        ...
        if prev_bev is not None:
            ...
            # breakpoint()
            num_frames = prev_bev.size(0)
            if num_frames != 1:
                prev_bev = torch.cat([prev_bev, bev_query], 0)
                shift_ref_2d = shift_ref_2d.repeat(num_frames, 1, 1, 1)
        ...
        if self.uncertainty_head is not None and uncertainty_embed is not None:
            uncertainty = self.uncertainty_head(uncertainty_embed).unsqueeze(1)
            prev_weights = (1 - uncertainty)
        ...
        for lid, layer in enumerate(self.layers):
            output = layer(
                ...
                *args,
                ...
                prev_bev=prev_bev,
                prev_weights=prev_weights if self.uncertainty_head is not None and uncertainty_embed is not None else None,
                uncertainty_embed=uncertainty_embed,
                flow=flow_down_sampling,
                au=au,
                **kwargs)
