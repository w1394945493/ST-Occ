...
class Metric_mIoU():
    ...
    def add_batch(self,semantics_pred,semantics_gt,mask_lidar,mask_camera, sn=None, token=None):
        self.cnt += 1
        ...
        if sn is not None and token is not None:
            if sn not in self.sample_miou:
                self.sample_miou[sn] = {}
            self.sample_miou[sn][token] = self.per_class_iu(_hist)
    ...
    def count_miou(self):
        ...
        # if len(self.sample_miou) > 0:
        #     import os
        #     os.makedirs(self.out_dir, exist_ok=True)
        #     import pickle
        #     with open(f'{self.out_dir}/per_frame_miou.pkl', 'wb') as file:
        #         pickle.dump(self.sample_miou, file)
