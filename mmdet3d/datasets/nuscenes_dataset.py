...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
                 ...
                 global_local_occupancy_path=False,
                ):
    ...
    def _set_sequence_group_flag(self):
        ...
        if self.sequences_split_num != 1:
            if self.sequences_split_num == 'all':
                ...
                # print(f'Original flag: {len(np.bincount(self.flag))}, New flag: {len(np.bincount(new_flags))}, Split num: {self.sequences_split_num}')
    ...
    def evaluate_flickering(self, results, out_dir, logger):
        ...
        For each frame, STCV is the number of classification changes of the
        current prediction w.r.t. the aggregated past predictions at the same
        global location, normalized by the number of non-free voxels.
        mSTCV (mean SpatioTemporal Classification Variability, see the ST-Occ
        paper, arXiv:2508.04705) averages STCV over all frames and is reported
        as a fraction (multiply by 100 for the percentage used in the paper).
        """
        cnt = 0
        percentage_cnt = 0
        stcv_cnt = 0
        frame_cnt = 0
        processed_scene = []
        dist_cnt = np.zeros(19)
        for i in range(len(results)):
            scenes = list(results[i]['flickering']['gt'].keys())
            for scene in scenes:
                if scene in processed_scene:
                    continue
                processed_scene.append(scene)
                gt = results[i]['flickering']['gt'][scene]
                pred = results[i]['flickering']['pred'][scene]
                total = results[i]['flickering']['total'][scene]
                total_non_free = results[i]['flickering']['total_non_free'][scene]
                gt_dist = results[i]['flickering']['gt_dist'][scene]
                pred_dist = results[i]['flickering']['pred_dist'][scene]
                for frame_token in gt.keys():
                    cnt += pred[frame_token] - gt[frame_token]
                    percentage_cnt += (pred[frame_token] - gt[frame_token]) / total[frame_token]
                    # per-frame STCV: changes normalized by non-free voxels
                    stcv_cnt += (pred[frame_token] - gt[frame_token]) / total_non_free[frame_token]
                    dist_cnt += pred_dist[frame_token] - gt_dist[frame_token]

                    frame_cnt += 1

        average_flickering_cnt = cnt / frame_cnt
        average_flickering_percentage = percentage_cnt / frame_cnt
        mSTCV = stcv_cnt / frame_cnt
        average_dist = dist_cnt / frame_cnt
        average_dist_percentage = average_dist / np.sum(average_dist)
    ...
    def evaluate_occupancy(self, occ_results, runner=None, show_dir=None, save=False, use_global_local=False, use_dynamic_mask=False,out_dir=None, **eval_kwargs):
        ...
        for occ_pred_w_index in tqdm(occ_results):
            ...
            if use_global_local:
                if use_dynamic_mask:
                    occupancy_file_path = osp.join(self.global_local_occupancy_path, scene_name, sample_token, 'label_local_dm_pu.npz')
                else:
                    occupancy_file_path = osp.join(self.global_local_occupancy_path, scene_name, sample_token, 'label_local.npz')
            else:
