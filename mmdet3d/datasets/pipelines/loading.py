...
class LoadOccVelocityMap(object):
    def __init__(self, x_min=-40, x_max=40, 
                 y_min=-40, y_max=40, 
                 z_min=-1, z_max=5.4,
                 voxel_size=0.4,
                 enlarge=1.0,
        ...
        self.z_enlarge_ratio = z_enlarge_ratio
        ...
        # self.speed_grid = torch.zeros((*self.grid_shape, 2))
        
    def get_voxel_index(self, point, axis_min, voxel_size):
        """Convert a 3D point to voxel indices."""
        # return ((point - axis_min) / voxel_size).long()
        return torch.round((point - axis_min) / voxel_size).long()
    
    def rotation_matrix_yaw(self, yaw):
        """Create a 2D rotation matrix for yaw."""
        cos_yaw = torch.cos(yaw)
        sin_yaw = torch.sin(yaw)
        return torch.tensor([[cos_yaw, -sin_yaw], [sin_yaw, cos_yaw]])
    ...
    def assign_speed_to_voxels_yaw(self, bbox, speed_grid):
        """Transform the bounding box with rotation and assign speed values to the voxels."""
        x, y, z, l, w, h, rots, vx, vy = bbox
        l *= self.enlarge
        w *= self.enlarge
        # h *= self.enlarge

        # rots = torch.tensor(-3.14/4).float()

        # Generate the rotation matrix
        rot_mat = self.rotation_matrix_yaw(rots)

        # Define the local corner points of the bounding box (centered at origin)
        ...
        # Rotate the corners and shift to (x, y)
        rotated_corners = corners @ rot_mat.T + torch.tensor([x, y])

        # Find the axis-aligned bounding box (AABB) around the rotated corners
        x_min_bbox, x_max_bbox = rotated_corners[:, 0].min(), rotated_corners[:, 0].max()
        y_min_bbox, y_max_bbox = rotated_corners[:, 1].min(), rotated_corners[:, 1].max()

        # Convert AABB to voxel indices
        x_start = self.get_voxel_index(x_min_bbox, self.x_min, self.voxel_size).clamp(0, self.grid_shape[0] - 1)
        x_end = self.get_voxel_index(x_max_bbox, self.x_min, self.voxel_size).clamp(0, self.grid_shape[0] - 1)
        y_start = self.get_voxel_index(y_min_bbox, self.y_min, self.voxel_size).clamp(0, self.grid_shape[1] - 1)
        y_end = self.get_voxel_index(y_max_bbox, self.y_min, self.voxel_size).clamp(0, self.grid_shape[1] - 1)

        # Determine the voxel indices along the z-dimension
        if self.enlarge > 1.0:
            z_range = torch.arange(z - h / 2 - h * (self.enlarge - 1) * (1 - self.z_enlarge_ratio), z + h / 2 + h * (self.enlarge - 1) * self.z_enlarge_ratio, self.voxel_size)
        else:
            z_range = torch.arange(z - h / 2, z + h / 2, self.voxel_size)
        z_indices = self.get_voxel_index(z_range, self.z_min, self.voxel_size)

        # Iterate through the voxels within the AABB and assign speed if inside the polygon
        for i in range(x_start, x_end + 1):
            for j in range(y_start, y_end + 1):
                voxel_center = torch.tensor([
                    self.x_min + (i + 0.5) * self.voxel_size,
                    self.y_min + (j + 0.5) * self.voxel_size,
                ])

                if self.is_point_in_polygon(voxel_center, rotated_corners):
                    for z_idx in z_indices:
                        if 0 <= z_idx < self.grid_shape[2]:  # Ensure within z-boundaries
                            speed_grid[i, j, z_idx] = torch.tensor([vx, vy]).float()
    
    def __call__(self, results):
        
        speed_grid = torch.zeros((*self.grid_shape, 2))
        bboxes = torch.tensor(results['ann_infos'][0])
        for bbox in bboxes:
            self.assign_speed_to_voxels_yaw(bbox, speed_grid)
        
        
        # augmentation and align coordinate
        speed_grid = speed_grid.permute(2, 0, 1, 3)
        speed_grid = torch.rot90(speed_grid, 1, [1, 2])
        speed_grid = torch.flip(speed_grid, [1])
        speed_grid = speed_grid.permute(1, 2, 0, 3)
        
        if results['rotate_bda'] != 0:
            speed_grid = speed_grid.permute(2, 0, 1, 3)
            for channel in range(speed_grid.shape[-1]):
                speed_grid[..., channel] = rotate(speed_grid[..., channel], -results['rotate_bda'], fill=0)
            speed_grid = speed_grid.permute(1, 2, 0, 3)
            # speed_grid = rotate(speed_grid, -results['rotate_bda'], fill=0).permute(1, 2, 0, 3)

        if results['flip_dx']:
            speed_grid = torch.flip(speed_grid, [1])

        if results['flip_dy']:
            speed_grid = torch.flip(speed_grid, [0])
        
        results['gt_speed_map'] = speed_grid
        
        return results

@PIPELINES.register_module()
class LoadFlowFromVelocity(object):
    def __init__(self, x_min=-40, x_max=40,
                 y_min=-40, y_max=40,
                 z_min=-1, z_max=5.4,
                 voxel_size=0.4,
    ...
    def assign_speed_to_voxels_yaw(self, bbox, speed_grid):
        ...
        # Determine the voxel indices along the z-dimension
        if self.enlarge > 1.0:
            z_range = torch.arange(z - h / 2 - h * (self.enlarge - 1) * (1 - self.z_enlarge_ratio),
                                   z + h / 2 + h * (self.enlarge - 1) * self.z_enlarge_ratio, self.voxel_size)
        else:
            z_range = torch.arange(z - h / 2, z + h / 2, self.voxel_size)
        z_indices = self.get_voxel_index(z_range, self.z_min, self.voxel_size)
        # Iterate through the voxels within the AABB and assign speed if inside the polygon
    ...
    def __call__(self, results):
        ...
        bboxes = torch.tensor(results['ann_infos'][0])
        for bbox in bboxes:
            self.assign_speed_to_voxels_yaw(bbox, speed_grid) # W H Z 2 (x y z)
        # augmentation and align coordinate
        speed_grid = speed_grid.permute(1, 0, 2, 3)
        speed_grid = torch.flip(speed_grid, [1])
        speed_grid[..., 0], speed_grid[..., 1] = speed_grid[..., 1], -speed_grid[..., 0]
        if results['rotate_bda'] != 0:
            speed_grid = speed_grid.permute(2, 0, 1, 3)
            for channel in range(speed_grid.shape[-1]):
                speed_grid[..., channel] = rotate(speed_grid[..., channel], -results['rotate_bda'], fill=0)
