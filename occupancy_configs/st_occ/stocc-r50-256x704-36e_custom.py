...
model = dict(
    ...
    uncertainty_config=dict(
        global_vis_cnt=True,
        global_vis_cnt_alpha=1,
        global_distance=True,
        global_distance_alpha=20,
        global_distance_max=60,
        global_prev_distribution=True,
        global_distribu_decay=2,
    ),
    relative_scale=True,
    implicit_uncertainty=True,
    embed_feature_uncertainty=True,
    w_au=True,
    au_ef=True,
    disable_flow_init=True,
    temporal_encoder=dict(
        type='VoxelFormerEncoder',
        uncertainty_head=dict(
                        type='UncertaintyEstimator_w_feature_au',
                        input_size=22,
                        hidden_size=64,),
        num_layers=num_tsa_layers,
        ...
        transformerlayers=dict(
            ...
            attn_cfgs=[
                dict(
                    type='VoxelTemporalSelfAttention_w_Uncertainty',
...
occupancy_path = "/c20250502/wangyushen/Datasets/occ3d_nuscenes/gts"
...
train_pipeline = [
    ...
    dict(type='LoadFlowFromVelocity'),
    dict(type='DefaultFormatBundle3D', class_names=class_names),
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_occupancy', 'gt_depth', 'gt_flow'
                               ])
]
...
test_data_config = dict(
    ...
    ann_file=data_root + 'bevdetv4-nuscenes_infos_global_val.pkl',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'bevdetv4-nuscenes_infos_global_train.pkl',
...
custom_hooks = [
    ...
    dict(
        ...
        temporal_start_iter=num_iters_per_epoch * 3,
    ),
]
